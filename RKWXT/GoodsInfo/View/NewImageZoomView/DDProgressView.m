...
//iKaola WebView loadProgress webview 进度
- (void)setWebViewProgress:(float)webViewProgress
{
    /* iKaola WebView loadProgress
     * 判断进度是否会 减少 如果 减少则不赋值
     */
    if (webViewProgress > self.progress) {
        self.progress = webViewProgress;
    }
}
...
- (void)setProgress:(float)theProgress
{
    ...
    if (self.automaticHidden && 1 == progress) {
        [self hiddenAnimated];
    }
...
//修改尺寸 rect 修改圆角 radius
//	rect = CGRectInset(rect, 1.0f, 1.0f) ;
    rect = CGRectInset(rect, 0, 0.0f) ;
