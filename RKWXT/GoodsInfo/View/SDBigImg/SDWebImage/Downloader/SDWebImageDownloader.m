...
- (void)connection:(NSURLConnection *)aConnection didReceiveResponse:(NSURLResponse *)response
{
    if ([((NSHTTPURLResponse *)response) statusCode] >= 400)
    {
        ...
        imageContentLength = [(NSHTTPURLResponse *) response expectedContentLength];
    }
}
...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    long long dataLength = [imageData length];
    if ([delegate respondsToSelector:@selector(imageDownloader:totalBytesWritten:expectedTotalBytes:)])
    {
        objc_msgSend(delegate, @selector(imageDownloader:totalBytesWritten:expectedTotalBytes:), self, dataLength, imageContentLength);
    }
}
...
//拓展 获取图片数据总长度
- (long long)getImageTotalDateLength
{
