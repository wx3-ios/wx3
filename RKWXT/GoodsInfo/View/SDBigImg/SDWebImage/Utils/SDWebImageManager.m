...
-(void)imageDownloader:(SDWebImageDownloader *)downloader totalBytesWritten:(long long)totalBytesWritten expectedTotalBytes:(long long)expectedTotalBytes
{
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        NSUInteger uidx = (NSUInteger)idx;
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:totalBytesWritten:expectedTotalBytes:)])
            {
                objc_msgSend(delegate, @selector(webImageManager:totalBytesWritten:expectedTotalBytes:), self, totalBytesWritten, expectedTotalBytes);
            }
        }
    }
}
...
- (float)getImageDidDownloadProgressWithUrl:(NSURL *)url
{
    ...
    for (SDWebImageDownloader *downloader in downloaders) {
        ...
        if ([[url absoluteString] isEqualToString:[downloader.url absoluteString]]) {
            ...
            totalLength = [downloader getImageTotalDateLength];
            break;
        }
    }
    if (totalLength <= 0) {
    ...
    float progress = didReceiveLength / (float)totalLength;
    return progress;
}
@end
