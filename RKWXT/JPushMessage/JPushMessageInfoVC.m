...
@interface JPushMessageInfoVC()<UIWebViewDelegate,DownSheetDelegate>{
    UIWebView *_webView;
    NSArray *menuList;
...
-(void)viewDidLoad{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@?phone=%@&msg_id=%ld",WXTWebBaseUrl,NormaleMessageUrl,userObj.user,(long)_messageID];
...
-(BOOL)jumpToParam:(NSDictionary*)paramDic{
    ...
    if([[paramDic objectForKey:@"go"] isEqualToString:@"messages"]){
        title = [[paramDic objectForKey:@"push_title"] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        abstract = [[paramDic objectForKey:@"abstract"] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        imgUrl = [[NSString alloc] initWithFormat:@"%@%@",AllImgPrefixUrlString,[paramDic objectForKey:@"message_home_img"]];
