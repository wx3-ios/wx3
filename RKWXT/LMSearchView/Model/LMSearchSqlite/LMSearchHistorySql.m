...
@implementation LMSearchHistorySql
@synthesize all;
...
-(void)createTable{
    NSString *sqlCreateTable = @"CREATE TABLE IF NOT EXISTS LMSEARCHHISTORY (ID INTEGER PRIMARY KEY AUTOINCREMENT, RecordName TEXT , RecordTime TEXT , RecordID TEXT , Other TEXT)";
    [self execSql:sqlCreateTable];
}

-(NSMutableArray *)selectAll{
    NSString *sqlQuery = @"SELECT * FROM LMSEARCHHISTORY";
...
-(BOOL)deleteLMSearchHistoryList:(NSString *)recordName{
    sqlite3_stmt *statement;
    //组织SQL语句
    static char *sql = "delete from LMSEARCHHISTORY  where RecordName = ?";
    //将SQL语句放入sqlite3_stmt中
    int success = sqlite3_prepare_v2(db, sql, -1, &statement, NULL);
    if (success != SQLITE_OK) {
        NSLog(@"Error: failed to delete:LMSEARCHHISTORY");
        sqlite3_close(db);
        return NO;
    }
...
-(BOOL)deleteAll{
    sqlite3_stmt *statement;
    //组织SQL语句
    static char *sql = "DROP TABLE LMSEARCHHISTORY";
    //将SQL语句放入sqlite3_stmt中
    int success = sqlite3_prepare_v2(db, sql, -1, &statement, NULL);
    if (success != SQLITE_OK) {
        NSLog(@"Error: failed to delete:LMSEARCHHISTORY");
