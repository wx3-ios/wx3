...
- (void)setState:(KPRState)state {
    [self setState:state animated:YES];
}

- (void)setState:(KPRState)state animated:(BOOL)animated{
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        _headerView = [[OrderListView alloc] initWithFrame:rect atTop:YES];
        _headerView.atTop = YES;
        [self addSubview:_headerView];
        
        rect = CGRectMake(0, frame.size.height, frame.size.width, frame.size.height);
        _footerView = [[OrderListView alloc] initWithFrame:rect atTop:NO];
