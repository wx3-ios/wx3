...
//            xoffset = xoffset<-_rightViewShowWidth?-_rightViewShowWidth:xoffset;
            if(xoffset < -_rightViewShowWidth){
                xoffset = -_rightViewShowWidth;
                if(![_coverButton superview]){
                    [_currentView addSubview:_coverButton];
                }
            }else{
...
//    /*平移带缩放效果的动画
    ...
    if (xoffset>0) {//向右滑的
		...
		CGRect rect = CGRectMake(_baseView.frame.size.width * (1 - scale) + xoffset, _baseView.bounds.origin.y + (totalHeight*(1 - scale) / 2), totalWidth * scale, totalHeight * scale);
        [_currentView setFrame:rect];
    }
    //*/
}
